...
export default function BillingPage() {
  ...
  return (
    <div className="mx-auto p-6 pb-10 space-y-6 flex flex-col bg-slate-50/50">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="bg-white p-1 rounded-2xl border border-slate-200 shadow-sm self-stretch lg:self-auto">
          <TabsList className="bg-transparent h-11">
            ...
            >
              <LayoutIcon size={18} className="mr-2" /> {t('tabs.workspace')}
            </TabsTrigger>
            <TabsTrigger
              value="history"
              className="px-6 rounded-xl data-[state=active]:bg-[#1392ec] data-[state=active]:text-white data-[state=active]:shadow-md font-bold text-sm transition-all data-[state=inactive]:cursor-pointer"
            >
              <ListIcon size={18} className="mr-2" /> {t('tabs.history')}
            </TabsTrigger>
          </TabsList>
        </Tabs>
      </div>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4 bg-white p-4 rounded-3xl border border-slate-100 shadow-sm mb-4">
              ...
              <div className="flex items-center gap-3 w-full sm:w-auto">
                <div className="flex items-center gap-2 text-slate-400 font-bold text-[10px] uppercase tracking-widest pl-2">
                  <FunnelIcon size={14} /> {t('filter.label')}
                </div>
                <Select
                  value={statusFilter}
                ...
                >
                  ...
                  <SelectContent position="popper" align="end" className="rounded-2xl border-slate-100 shadow-xl">
                    <SelectItem value="ALL_STATUS" className="rounded-xl focus:bg-slate-50 font-medium capitalize prose-sm">{t('filter.all_status')}</SelectItem>
                    <SelectItem value={InvoiceStatus.DRAFT} className="rounded-xl focus:bg-slate-50 font-medium capitalize prose-sm">{t('filter.draft')}</SelectItem>
                    <SelectItem value={InvoiceStatus.OPEN} className="rounded-xl focus:bg-slate-50 font-medium capitalize prose-sm">{t('filter.open')}</SelectItem>
                    <SelectItem value={InvoiceStatus.PAID} className="rounded-xl focus:bg-slate-50 font-medium capitalize prose-sm text-emerald-600">{t('filter.paid')}</SelectItem>
                    <SelectItem value={InvoiceStatus.CANCELLED} className="rounded-xl focus:bg-slate-50 font-medium capitalize prose-sm text-red-600">{t('filter.cancelled')}</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </div>
