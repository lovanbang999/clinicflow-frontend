            ...
            <div className="text-center sm:text-left flex-1">
              <div className="flex items-center gap-2 justify-center sm:justify-start mb-1">
                ...
                <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${
                  ...
                  {patient.status === 'active' || patient.isActive ? t('status.active').toUpperCase() : (patient.status || 'INACTIVE').toUpperCase()}
                </span>
              </div>
                      ...
                      {visits.length > 0 ? visits.slice(0, 3).map((visit, idx) => (
                        <div key={idx} className="relative border border-transparent">
                          ...
                          <div className="flex flex-col -mt-1">
                            ...
                            <span className="text-xs text-slate-500 mt-1">{format(new Date(visit.bookingDate), 'MMM dd, yyyy')} &bull; {visit.doctorName}</span>
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                      {visits.filter(v => v.medicalRecord).length > 0 ? visits.filter(v => v.medicalRecord).map((visit, idx) => (
                        <tr key={idx} className="hover:bg-slate-50/30">
                          <td className="px-4 py-3 font-medium text-slate-900 dark:text-slate-200">{format(new Date(visit.bookingDate), 'MMM dd, yyyy')}</td>
                  ...
                  {visits.filter(v => v.prescription).length > 0 ? visits.filter(v => v.prescription).map((visit, idx) => (
                    <div key={idx} className="bg-white dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800 p-4 shadow-sm">
                      <div className="flex justify-between items-start mb-2">
                        <span className="text-xs font-bold text-slate-500">{format(new Date(visit.bookingDate), 'MMM dd, yyyy')}</span>
