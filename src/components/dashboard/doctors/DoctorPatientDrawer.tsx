...
export function DoctorPatientDrawer({ patient, open, onOpenChange }: DoctorPatientDrawerProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="p-0 border-none shadow-none bg-transparent sm:max-w-none w-full h-full pointer-events-none flex justify-end">
        <div className={`
          pointer-events-auto h-full w-full sm:max-w-[500px] md:max-w-[650px] bg-white dark:bg-[#0c101a] shadow-[-20px_0_40px_-20px_rgba(0,0,0,0.1)] 
          flex flex-col border-l border-slate-200/60 dark:border-slate-800 animate-in slide-in-from-right duration-500 ease-out
        `}>
          {/* HEADER SECTION (HERO) */}
          <div className="relative overflow-hidden pt-12 pb-6 px-8 border-b border-slate-100 dark:border-slate-800 shrink-0">
            {/* Background elements */}
              ...
              <div className="flex-1 pt-1 space-y-4">
                <div className="space-y-1">
                  <div className="flex flex-wrap items-center justify-center md:justify-start gap-2.5">
                    <h2 className="text-3xl font-black tracking-tight text-slate-900 dark:text-slate-100 leading-tight">
                          ...
                          {historyData.visits.map((visit) => {
                            ...
                            return (
                              <div
                                key={visit.id}
                                className={`
                                  rounded-[24px] border transition-all duration-500 overflow-hidden group relative
