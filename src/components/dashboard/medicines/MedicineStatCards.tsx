...
import { type MedicineStats } from '@/lib/api/admin/admin-medicines';
...
export function MedicineStatCards({ stats, isLoading }: Props) {
  const t = useTranslations('adminMedicines.stats');
  ...
  const activePct =
    stats && stats.totalMedicines > 0
      ? Math.round((stats.activeMedicines / stats.totalMedicines) * 100)
      : 0;

  const cards = [
    {
      label: t('totalMedicines'),
      value: stats ? String(stats.totalMedicines) : '—',
      icon: PillIcon,
      bg: 'bg-blue-50 text-[#1392ec]',
      badge: null,
    },
    {
      label: t('activeMedicines'),
      value: stats ? String(stats.activeMedicines) : '—',
      icon: CheckCircleIcon,
      bg: 'bg-emerald-50 text-emerald-600',
      badge: stats && stats.totalMedicines > 0 ? (
    ...
    {
      label: t('inactiveMedicines'),
      value: stats ? String(stats.inactiveMedicines) : '—',
      icon: XCircleIcon,
      bg: 'bg-slate-50 text-slate-600',
      badge: null,
    },
    {
      label: t('outOfStockMedicines'),
      value: stats ? String(stats.outOfStockMedicines) : '—',
      icon: WarningCircleIcon,
      bg: 'bg-rose-50 text-rose-600',
      badge: stats && stats.outOfStockMedicines > 0 ? (
        <span className="flex items-center gap-1 text-rose-600 bg-rose-50 border border-rose-200 px-2 py-0.5 rounded-full text-xs font-bold animate-pulse">
          Alert
        </span>
      ) : null,
    },
